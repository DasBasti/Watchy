...
void go_to_sleep()
{
  if (!stay_active)
  {
    uint8_t sleep_time = 59 - rtc.getDateTimeComponent(DATETIME_SECOND);
    if (sleep_time == 0) // if zero seconds to next update. Do it state.now.
    {
...
{
  state.now->day = rtc.getDateTimeComponent(DATETIME_DAY_OF_MONTH);
  state.now->month = rtc.getDateTimeComponent(DATETIME_MONTH);
  state.now->minute = rtc.getDateTimeComponent(DATETIME_MINUTE);
  state.now->hour = rtc.getDateTimeComponent(DATETIME_HOUR);
  PRINTLN(rtc.getCurrentDateTime());
  PRINT("Battery: ");
  PRINTLN(state.battery);
