...
#define CHARACTERISTIC_UUID_CTS ((uint16_t)ESP_GATT_UUID_CURRENT_TIME)
...
#endif
class ctsCallback : public BLECharacteristicCallbacks
{
public:
    ctsCallback(BLE *ble)
    {
...
void ctsCallback::onWrite(BLECharacteristic *pCharacteristic)
{
    std::string rxData = pCharacteristic->getValue();
    if (_p_ble != NULL)
    {
...
//
// begin
bool BLE::begin(const char *localName = "Schmartwatch")
{
    ...
    pCurrentTimeCharacteristic = pService->createCharacteristic(
        CHARACTERISTIC_UUID_CTS,
        BLECharacteristic::PROPERTY_READ | BLECharacteristic::PROPERTY_WRITE);

    pCurrentTimeCharacteristic->setCallbacks(new ctsCallback(this));
    ...
    uint8_t hardwareVersion[4] = {2, 2, 1, 0};
    pVersionCharacteristic->setValue((uint8_t *)hardwareVersion, 4);
    pWatchFaceNameCharacteristic->setValue("7 Segment Chip");
...
void BLE::setCtsUpdateCb(void (*cb)(char *iso8601))
{
    if (cb)
        pCtsUpdateCallback = cb;
}

void BLE::callCtsUpdate(std::string str)
{
    if (!pCtsUpdateCallback)
        return;
